...
#如果没有文件就创建一个
try:
    f = open(r'/game_many_user.txt', encoding='utf-8')
except:
    file_name =r'/game_many_user.txt'
...
record_2 = [cishu,min_round,sum_round]
dic_a[name] = record_2
output = ''
for j in dic_a:
    output_2 = j + ' ' +  ' '.join(str(dic_a[j])) + '\n'
    output += output_2

#在新文件中记录结果
f = open(r'/game_many_user_report.txt','w',encoding='utf-8')
f.write(output)
f.close